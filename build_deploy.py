...
def trim_data(out: Path) -> None:
    ...
    # CVM daily NAV cache: ~130MB of monthly parquet, and every byte of it is a
    # re-download away. The DERIVED files stay (metrics + registry + meta, ~4MB), so
    # Brazil Funds' Screener, Managers and Flows tabs work the moment the bundle lands;
    # only the Fund tab's history chart waits for the first daily pull to refill this.
    nav = sorted((data / "signals" / "cvm_funds").glob("nav_*.parquet"))
    for p in nav:
        p.unlink(missing_ok=True); removed += 1
    if nav:
        _log(f"  dropped {len(nav)} CVM monthly NAV file(s) — regenerated by the daily pull")
